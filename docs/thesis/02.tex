...
Il termine Zero Trust fu coniato da John Kindervag nel 2010, durante il suo lavoro presso Forrester Research; le radici concettuali del modello affondano tuttavia in una critica più antica al modello perimetrale. L'idea tradizionale di sicurezza informatica era quella del castello con il fossato: una rete interna considerata sicura, protetta da un perimetro difensivo verso l'esterno.
...
L'elemento architetturale chiave introdotto in questa fase è la valutazione degli attributi ambientali, rappresentati specificamente da uno \textit{score di rischio} continuo generato dal modello di AI. Attraverso questo scambio di attributi, l'ABAC realizza concretamente il principio della valutazione continua del rischio.
...
La componente deterministica dell'algoritmo si basa sull'implementazione del modello di confidenzialità Bell-LaPadula (BLP) spiegato in precedenza.
...
Nel progetto questo principio è applicato nel controllo degli accessi attraverso le seguenti verifiche:
\begin{enumerate}
    \item il ruolo nel certificato e nel jwt devono coincidere
    \item il certificato è obbligatorio per i ruoli admin/manager
    \item nel Open Policy Agent vengono valutati gli attributi : categoria, livello BLP e rischio associato alla richiesta
\end{enumerate}
...
Nel progetto, i meccanismi di protezione sono basati su standard aperti e ampiamente verificati: TLS 1.3 per la cifratura delle comunicazioni, X.509 per i certificati, SHA-256 per gli hash di integrità. La sicurezza risiede nella segretezza delle chiavi private, non nella segretezza degli algoritmi.
...
Nel progetto, Envoy è configurato con \texttt{enable\_ja3\_fingerprinting: true} nel filtro TLS Inspector. Envoy   calcola il digest MD5 del JA3 ed espone la stringa JA3  come variabile di metadato e la inietta negli header della richiesta inoltrata al Security Orchestrator.
...
Nel progetto, nftables opera come firewall di rete posizionato tra il client esterno e il
  ...
  cui 2 catene principali, \texttt{input} e \texttt{output}, adottano tutte
  ...
  appartenente a connessioni già stabilite e il traffico TCP verso la porta di Envoy , loggando ogni pacchetto accettato o rifiutato
...
Snort può operare in 3 modalità: sniffer (stampa i pacchetti sulla console), packet logger (salva i pacchetti su disco) e network intrusion detection system (analizza i pacchetti e genera alert). Nel progetto, Snort è configurato in modalità NIDS con output su console, condivide lo stack di rete del container firewall tramite la direttiva \texttt{network\_mode: service:firewall}.

La regola custom configurata utilizza il preprocessore \texttt{sfportscan} per rilevare scansioni di porte, abilitato con il parametro \texttt{sense\_level: low} per minimizzare i falsi positivi. Regole Snort aggiuntive segnalano esplicitamente la rilevazione di un port scan TCP quando vengono rilevati più di 5 pacchetti SYN dalla stessa sorgente nell'arco di 5 secondi e tentativi di SQLinjection. Questo scenario è uno dei vettori di attacco testati esplicitamente nel progetto, con client di test che simulano la scansione rapida di un intervallo di porte.
...
Nel progetto sono presenti 2 istanze MongoDB con ruoli distinti. Il Security DB conserva i
  ...
  credenziali degli utenti con ruolo e flag di enrollment TPM (collection
