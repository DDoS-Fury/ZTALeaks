\documentclass[12pt, a4paper]{report}

% ============================================================
%  ENCODING E LINGUA
% ============================================================
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}          % Fondamentale per font europei
\usepackage[italian]{babel}

% ============================================================
%  A CAPO AUTOMATICO PER \texttt (path, codice, identificatori lunghi)
%  Evita che il testo a larghezza fissa (\texttt) sfori il margine destro.
% ============================================================
\usepackage[htt]{hyphenat}        % consente l'a-capo anche nei \texttt
\sloppy                            % preferisce spaziatura più larga a sforare il margine

% Crea un comando dedicato alle stringhe lunghe
\newcommand{\code}[1]{\texttt{\seqsplit{#1}}}

% ============================================================
%  LAYOUT E GEOMETRIA
% ============================================================
\usepackage[
    a4paper,
    top=3.5cm,
    bottom=3.5cm,
    left=2.5cm,
    right=2.5cm
]{geometry}

\usepackage{parskip}              % Spazio tra paragrafi, niente rientro
\setlength{\parindent}{0pt}       % Rientro primo paragrafo = 0

% ============================================================
%  INTESTAZIONI E PIÈ DI PAGINA
% ============================================================
\usepackage{fancyhdr}
\usepackage{lastpage}

\pagestyle{fancy}

\renewcommand{\chaptermark}[1]{\markboth{\MakeUppercase{Capitolo \thechapter}}{}}
\renewcommand{\sectionmark}[1]{\markright{\MakeUppercase{#1}}}

\fancyhf{}
\renewcommand{\headrulewidth}{0.4pt}   % Linea sottile sopra
\fancyhead[L]{\small\leftmark}         % Capitolo corrente a sinistra
\fancyhead[R]{\small\rightmark}        % Sezione corrente a destra
\fancyfoot[C]{\thepage\ di \pageref{LastPage}}  % "N di TOT" al centro

% Stile per le pagine iniziali di capitolo (plain)
\fancypagestyle{plain}{
    \fancyhf{}
    \renewcommand{\headrulewidth}{0pt}
    \fancyfoot[C]{\thepage\ di \pageref{LastPage}}
}

% ============================================================
%  TITOLI DI CAPITOLO E SEZIONE
% ============================================================
\usepackage{titlesec}
\usepackage{hyperref}

% Capitolo: numero e titolo sulla stessa riga, senza "Capitolo"
\titleformat{\chapter}[hang]
    {\normalfont\huge\bfseries}   % Formato
    {\thechapter}                 % Solo il numero
    {1em}                         % Spazio numero-titolo
    {}
\titlespacing*{\chapter}{0pt}{-20pt}{20pt}  % Riduce spazio bianco sopra

% Sezione
\titleformat{\section}
    {\normalfont\Large\bfseries}
    {\thesection}{1em}{}

% Sottosezione
\titleformat{\subsection}
    {\normalfont\large\bfseries}
    {\thesubsection}{1em}{}

% ============================================================
%  GRAFICA E FIGURE
% ============================================================
\usepackage{graphicx}
\graphicspath{{immagini/}}
\usepackage{float}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows.meta, positioning, calc, fit, backgrounds}

\captionsetup{
    font=small,
    labelfont=bf,
    format=hang
}

% ============================================================
%  TABELLE
% ============================================================
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{longtable}

% ============================================================
%  MATEMATICA
% ============================================================
\usepackage{amsmath}
\usepackage{amssymb}

% ============================================================
%  HYPERREF (sempre ultimo)
% ============================================================
\usepackage{hyperref}
\hypersetup{
    colorlinks  = true,
    linkcolor   = black,
    citecolor   = black,
    filecolor   = black,
    urlcolor    = blue,
    pdftitle    = {Sviluppo di una Zero Trust Architecture per la gestione di un impianto di energia nucleare},
    pdfauthor   = {Baldelli, Marcianesi, Tempera, Vianello},
    pdfsubject  = {Tesina Advanced Cybersecurity for IT — UNIVPM},
    pdfkeywords = {Sviluppo di una Zero Trust Architecture per la gestione di un impianto di energia nucleare}
}

% ============================================================
%  DOCUMENTO
% ============================================================
\begin{document}

% ----------------------------------------------------------
%  PAGINA DI COPERTINA
% ----------------------------------------------------------
\begin{titlepage}
    \centering

    \includegraphics[width=0.28\textwidth]{logo}

    \vspace{0.8cm}
    {\LARGE\bfseries Università Politecnica delle Marche\par}

    \vspace{0.3cm}
    {\large Facoltà di Ingegneria\par}

    \vspace{0.2cm}
    {\normalsize Dipartimento di Ingegneria dell'Informazione\par}

    \vspace{0.8cm}
    \rule{\textwidth}{0.4pt}    % Linea separatrice superiore
    \vspace{0.4cm}

    {\large Corso di Laurea Magistrale in\\
     Ingegneria Informatica e dell'Automazione\par}

    \vspace{0.8cm}

    {\LARGE\bfseries
     Sviluppo di una Zero Trust Architecture per la gestione di un impianto di energia nucleare\par}

    \vspace{0.4cm}
    \rule{\textwidth}{0.4pt}    % Linea separatrice inferiore

    \vfill

    % Docenti e Componenti del gruppo
    \noindent
    \begin{minipage}[t]{0.48\textwidth}
        \raggedright
        {\small\textbf{Docente}}\\[4pt]
        Prof.\ Luca Spalazzi
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \raggedleft
        {\small\textbf{Componenti del gruppo}}\\[4pt]
        Fabio Tempera\\
        Luca Marcianesi\\
        Gabriele Vianello\\
        Gianluca Baldelli
    \end{minipage}

    \vspace{1.5cm}

    {\normalsize Anno Accademico 2025--2026\par}

\end{titlepage}

% ----------------------------------------------------------
%  INDICI
% ----------------------------------------------------------
\tableofcontents
\listoffigures
%\listoftables   

\newpage

% ----------------------------------------------------------
%  CAPITOLI
% ----------------------------------------------------------
\include{01}
\include{02}
\include{03}
\include{04}
\include{05}
\include{06}
\include{07}
\include{08}
\end{document}